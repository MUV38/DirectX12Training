...
cbuffer ParticleGlobalParameter : register(b1)
{
    float4 VelocityParam;
}
...
float PerlinNoise(float3 v)
{
  ...
  // Permutations
    ...
    //float4 s0 = vec4(lessThan(b0,0.0))*2.0f - 1.0f;
    //float4 s1 = vec4(lessThan(b1,0.0))*2.0f - 1.0f;
...
float3 PerinNoise3d(float3 vec)
{
...
float3 CurlNoise(float3 pos)
{
    ...
    float3 p_x0 = PerinNoise3d(pos - dx);
    float3 p_x1 = PerinNoise3d(pos + dx);
    float3 p_y0 = PerinNoise3d(pos - dy);
    float3 p_y1 = PerinNoise3d(pos + dy);
    float3 p_z0 = PerinNoise3d(pos - dz);
    float3 p_z1 = PerinNoise3d(pos + dz);
...
{
	uint index = GroupThreadID.x + GroupThreadID.y * THREAD_X + GroupID.x * (THREAD_X * THREAD_Y);

    // カールノイズで速度を求めて適用位置に適用するだけ
	float3 pos = ParticleDataBuffer[index].position;
    float3 velocity = CurlNoise(pos);
	pos += velocity * Time.y * VelocityParam.x;

	ParticleDataBuffer[index].position = pos;
}