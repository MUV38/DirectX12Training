...
void FullScreenQuad::Setup(ID3D12Device* device, const SetupParam& param)
{
	...
	// シェーダー
	{
		...
		hr = D3D12Util::CompileShaderFromFile(ASSET_SHADER_ROOT"FullScreenQuad_vs.hlsl", L"vs_6_0", m_vs, errBlob);
		if (FAILED(hr))
		{
		...
		if (!m_ps)
		{
			hr = D3D12Util::CompileShaderFromFile(ASSET_SHADER_ROOT"FullScreenQuad_ps.hlsl", L"ps_6_0", m_ps, errBlob);
			if (FAILED(hr))
			{
				OutputDebugStringA(static_cast<const char*>(errBlob->GetBufferPointer()));
			}
